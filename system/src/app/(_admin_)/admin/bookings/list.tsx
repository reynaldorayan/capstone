...
export default function Bookings() {
  ...
  const {
    ...
    formState: { errors: approveErrors, isSubmitting: approveSubmitting },
  } = useForm<ApproveFormInput>({
  ...
  } = useSWR<BookingWithRelation[]>("/api/bookings", (url: string) =>
    ...
    try {
      ...
      if (response.ok) {
        Swal.fire({
          ...
          validationMessage: "Booking has been approved successfully.",
  ...
  const onSubmitReject = async (data: RejectFormInput) => {
    try {
      const response = await fetch(
      ...
      if (response.ok) {
        Swal.fire({
          ...
          validationMessage: "Booking has been rejected successfully.",
          showConfirmButton: true,
          timer: 1500,
        }).finally(() => {
          mutateBookings();
          ...
          <div className="flex flex-col gap-1">
            ...
            {item.status === "REJECTED" && item.refundReceipt === null && (
              <span className="text-sm text-cyan-600">
                Please upload your reciept to make refund completed.
              </span>
            )}
          </div>
        );
  ...
  return (
    <div className="h-screen px-4 mt-10 flex flex-col gap-5 sm:container sm:mx-auto">
      ...
      {selectedBooking && (
        <>
          <Modal
            title={`Reject booking #${selectedBooking.bookingNo}`}
            isOpen={showRejectModal}
            onClose={closeRejectModal}
          >
            <p>
              Reject this booking #{selectedBooking.bookingNo}? This action is
              irreversible.
            </p>
            ...
            <form onSubmit={handleSubmitReject(onSubmitReject)}>
              <Textarea
                label="Reason"
                {...rejectRegister("reasonForRejection")}
                error={rejectErrors.reasonForRejection?.message}
              />

              <Button
                isLoading={rejectSubmitting}
                isDisabled={rejectSubmitting}
                className="w-fit font-medium mt-3"
                type="submit"
              >
                Confirm Reject
              </Button>
          ...
          <Modal
            title={`Approve booking #${selectedBooking.bookingNo}`}
            isOpen={showApproveModal}
            onClose={closeApproveModal}
          >
            <p>
              Confirm this booking #{selectedBooking.bookingNo}? This action is
              irreversible.
            </p>

            <form onSubmit={handleSubmitApprove(onSubmitApprove)}>
              <input
                {...approveRegister("bookingId")}
                value={selectedBooking.id}
                hidden
              />
              <Button
                isLoading={approveSubmitting}
                isDisabled={approveSubmitting}
                className="w-fit font-medium mt-3"
                type="submit"
              >
                Confirm Approve
          ...
          <Modal
            title={`Refund reciept for booking #${selectedBooking.bookingNo}`}
            isOpen={showRefundModal}
            onClose={closeRefundModal}
          >
            <div className="flex flex-col gap-4">
              <div className="relative">
                <div>
                  <h2>Upload your reciept for the refund of the payment</h2>
                </div>
